...
class Agent:
	...
	def raycast_obstacles(self, obstacles, distance):
		
		for i in range(0, distance):
			for obstacle in obstacles:
								
				x = self.pos.x + math.cos(math.radians(self.heading)) * i;
				y = self.pos.y + math.sin(math.radians(self.heading)) * i;
				
				if x >= obstacle.x1 and x <= obstacle.x2:
					if y >= obstacle.y1 and y <= obstacle.y2:
						return i;
						
		return False;
	
	def raycast(self, agents, distance):
		...
		#for agent in agents:
		#	if agent != self:	
		#		boundingBoxes.append(CRect(agent.pos.x, agent.pos.y, BBOX_WIDTH));
				
		#for box in boundingBoxes:
		#	self.environment.canvas.create_rectangle(box.tl.x, box.tl.y, box.br.x, box.br.y, outline="blue");
			
				#print(str(agent.pos.x) + "... " + str(agent.pos.y));
				#self.environment.canvas.create_line(agent.pos.x, agent.pos.y, agent.pos.x + 100, agent.pos.y + 100, fill="blue");
		...
		for i in range(0, distance):
			for agent in agents:
				...
				box = CRect(agent.pos.x, agent.pos.y, BBOX_WIDTH);
				
				x = self.pos.x + math.cos(math.radians(self.heading)) * i;
				y = self.pos.y + math.sin(math.radians(self.heading)) * i;
				
				if x >= box.tl.x and x <= box.tr.x:
					if y >= box.tl.y and y <= box.br.y:
						return agent;
						
		return None;
		
		#self.environment.canvas.create_line(startPos.x, startPos.y, endPos.x, endPos.y, fill="red");
